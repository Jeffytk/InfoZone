...
function getPosts() {
    return JSON.parse(localStorage.getItem('astropost')) || [];
}

function savePost(post) {
    const posts = getPosts();
    posts.unshift(post);
    localStorage.setItem('astropost', JSON.stringify(posts));
}
...
function displaySinglePost(postId) {
    ...
    postContainer.innerHTML = `
        <article class="single-post">
            ...
            <a href="${window.location.pathname}" class="back-link">← Volver al foro</a>
        </article>
